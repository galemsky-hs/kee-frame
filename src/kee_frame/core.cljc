...
(def valid-option-key? #{:router :hash-routing? :routes :routes-options :process-route :debug? :debug-config
                         ...
                         :screen :scroll :base-path :navigator :controllers-enabled-fn :route-change-event :not-found :log :global-interceptors})

;; :routes-options
;; :base-path :navigator :controllers-enabled-fn
